...
class SnowFlakeNet(nn.Module):
    ...
    def get_loss(self, pcds_pred, complete_gt, epoch=1, **kwargs):
        """loss function
        Args
            pcds_pred: List of predicted point clouds, order in [Pc, P1, P2, ..., Pn, partial_input]
        """
        # Last element is the partial input appended during training
        partial_input = pcds_pred[-1]
        pcd_stages = pcds_pred[:-1]  # All stages except the appended partial

        Pc = pcd_stages[0]
        P_fine = pcd_stages[-1]

        # Build ground truth at each resolution
        gts = [complete_gt]
        for stage in reversed(pcd_stages[:-1]):
            gts.insert(0, fps(gts[0], stage.shape[1]))

        # CD loss at each stage
        cd_losses = [self.loss_func_CD(pred, gt) for pred, gt in zip(pcd_stages, gts)]
        partial_matching = self.loss_func_PM(partial_input, P_fine)

        loss_coarse = (sum(cd_losses[:-1]) + partial_matching) * 1000
        loss_fine = cd_losses[-1] * 1000
