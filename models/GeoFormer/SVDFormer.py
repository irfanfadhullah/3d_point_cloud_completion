...
import time
from models.GeoFormer.model_utils import *
...
class SDG(nn.Module):
    def __init__(self, channel=128,ratio=1,hidden_dim = 512,dataset='ShapeNet'):
        ...
        # Use a simple L2 distance as proxy for structure analysis
        self._cd_l1 = ChamferDistanceL1()
    ...
    def forward(self, local_feat, coarse,f_g,partial):
        ...
        # Structure Analysis — compute per-point nearest-neighbor distance (replaces chamfer_3DDist)
        coarse_t = coarse.transpose(1, 2).contiguous()  # (B, N, 3)
        partial_t = partial.transpose(1, 2).contiguous()  # (B, M, 3)
        _dists = torch.cdist(coarse_t, partial_t)  # (B, N, M)
        half_cd = _dists.min(dim=2)[0] / self.sigma  # (B, N)
...
class SVFNet(nn.Module):
    ...
    def get_img_layers(backbone, feat_size):
        ...
        from models.GeoFormer.resnet import _resnet, BasicBlock
