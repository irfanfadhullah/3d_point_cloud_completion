...
from models.MAENet.decoder.utils.utils import *
import torch
...
class Propagation(nn.Module):
    def __init__(self, embed_dim=128, num_p0=512, n_knn=20, radius=1, up_factors=[2], interpolate='nearest', attn_channel=True):
        ...
        self.seed_fea = MLP_Res(in_dim=3, hidden_dim=128)

    def forward(self, pcd, seed):
        pred_pcds = []
        seed = seed.permute(0, 2, 1).contiguous()
        seed_feat = self.seed_fea(seed)
        K_prev = None
