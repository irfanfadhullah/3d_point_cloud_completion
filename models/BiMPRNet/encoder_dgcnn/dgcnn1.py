...
class DGCNN(nn.Module):
    ...
    def forward(self, x):  # x --> (B X F X N)
        ...
        feat_max = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)
        feat_avg = F.adaptive_avg_pool1d(x, 1).view(batch_size, -1)
        feat_gf = torch.cat((feat_max, feat_avg), dim=1)

        # Decoder coarse input
        coarse_pcd = self.fc(feat_gf).reshape(batch_size, -1, 256)
