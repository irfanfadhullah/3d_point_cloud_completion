...
from models.BiMPRNet.decoder.dec_net import Decoder_Network
from models.BiMPRNet.encoder_dgcnn.dgcnn import DGCNN
from models.BiMPRNet.encoder_image.resnet import ResNet
from models.BiMPRNet.config import params
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.self_attn1 = nn.MultiheadAttention(p.d_attn, p.num_heads, batch_first=True)
        self.layer_norm2 = nn.LayerNorm(p.d_attn)

        self.cross_attn2 = nn.MultiheadAttention(p.d_attn, p.num_heads, batch_first=True)
    ...
    def forward(self, x_part, view):
        ...
        pc_feat = self.pc_encoder(x_part)  # B x F x N(B 256 128)
        im_feat = self.im_encoder(view)  # B x F x N(B 256 196)

        im_feat = im_feat.permute(0, 2, 1)  # B x N x F(B 196 256)
        pc_feat = pc_feat.permute(0, 2, 1)  # B x N x F(B 128 256)

        x, _ = self.cross_attn1(pc_feat, im_feat, im_feat)
        pc_feat = self.layer_norm1(x + pc_feat)  # B x N x F(B 128 256)
