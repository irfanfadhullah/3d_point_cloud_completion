...
try:
    ...
    # Pure-Python fallback when MDS CUDA extension is unavailable
    class _MDS_fallback:
        @staticmethod
        def minimum_density_sample(xyz, n_sample, mean_mst_dis):
            """Fallback: use farthest point sampling instead of MDS."""
            B, N, _ = xyz.shape
            device = xyz.device
        ...
        def gather_operation(features, idx):
            """Gather features by index. features: (B, C, N), idx: (B, M) -> (B, C, M)"""
            idx_long = idx.long()
            return torch.gather(features, 2, idx_long.unsqueeze(1).expand(-1, features.size(1), -1))
    
    MDS_module = _MDS_fallback()
