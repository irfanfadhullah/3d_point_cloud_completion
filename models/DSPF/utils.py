...
class Fp_MLP(nn.Module):
    def __init__(self,in_channel=768,out_channel=256, kernel_size=(1, 1), stride=(1, 1)):
        super(Fp_MLP, self).__init__()
        self.fp_mlp1 = nn.Conv2d(768, 256, kernel_size, stride=stride)
        self.fp_mlp2 = nn.Conv2d(256, 256, kernel_size, stride=stride)
        self.fp_activate=torch.relu

    def forward(self,input):#input (b,768,128)

        if input.shape[1]!=768:
            raise ValueError("输入张量的第二维度不匹配")

        input=input.unsqueeze(3)#(b,768,128,1)
        Fp1=self.fp_mlp1(input)#(b,256,128,1)
        Fp1=self.fp_activate(Fp1)#(b,256,128,1)
        Fp1 = self.fp_mlp2(Fp1)  # (b,256,128,1)
        Fp1 = torch.max(Fp1, 3)[0] #(b,256,128)
        return Fp1 #(b,256,128)


class Ff_MLP(nn.Module):
    def __init__(self, in_channel=768, out_channel=512, kernel_size=(1, 1), stride=(1, 1)):
        super(Ff_MLP, self).__init__()
        self.ff_mlp1 = nn.Conv2d(768, 512, kernel_size, stride=stride)
        self.ff_mlp2 = nn.Conv2d(512, 512, kernel_size, stride=stride)
        self.ff_activate = torch.relu
    ...
    def forward(self, input):  # input (b,768,128)
        ...
        input = input.unsqueeze(2)  # (b,768,1,128)
        Ff1 = self.ff_mlp1(input) # (b,512,1,128)
        Ff1 = self.ff_activate(Ff1)  # (b,512,1,128)
        Ff1 = self.ff_mlp2(Ff1)  # (b,512,1,128)
        Ff1 = torch.max(Ff1, 3)[0]  # (b,512,1)
        return Ff1  # (b,512,1)
...
class PointNet_SA_Module_KNN(nn.Module):
    ...
    def forward(self, xyz, points, idx=None):
        ...
        """
        if self.group_all:
            # print("new_points.shapegrouall0000000:",points.shape)
            new_xyz, new_points, idx, grouped_xyz = sample_and_group_all(xyz, points, self.use_xyz)
            # print("new_points.shapegrouall1111:", new_points.shape)
            # print("new_points.shapegrouall1111:", new_points.shape)
        else:
            new_xyz, new_points, idx, grouped_xyz = sample_and_group_knn(xyz, points, self.npoint, self.nsample, self.use_xyz, idx=idx)
            # print("new_points.shape__no__grouall:", new_points.shape)

        # print("new_points.shape1", new_points.shape)
        new_points = self.mlp_conv(new_points)
        # print("new_points.shape2", new_points.shape)
        new_points = torch.max(new_points, 3)[0]
        # print("new_points.shape3", new_points.shape)
