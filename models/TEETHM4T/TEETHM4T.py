...
from models.TEETHM4T.Transformer_utils import *
from utils import misc
import torch.nn.functional as F

class FeatureAlignmentAugmentation(nn.Module):
    def __init__(self, feature_dim, num_fragments, cache_size, augmentation_weight=0.3):
        """
        FAA module as described in FACNet.
        :param feature_dim: Dimension of high-dimensional input features.
        :param num_fragments: Number of fragments to divide features into.
        :param cache_size: Number of features stored in the feature cache.
        :param augmentation_weight: Lambda controlling augmentation weight.
        """
        super(FeatureAlignmentAugmentation, self).__init__()
        assert feature_dim % num_fragments == 0, "Feature dimension must be divisible by number of fragments."

        self.feature_dim = feature_dim
        self.num_fragments = num_fragments
        self.fragment_dim = feature_dim // num_fragments
        self.augmentation_weight = augmentation_weight

        # Learnable feature cache
        self.feature_cache = nn.Parameter(torch.randn(cache_size, self.fragment_dim))

    def forward(self, features_gt, features_partial):
        """
        Forward pass of the FAA module with stop-gradient steps.
        :param features_gt: Ground-truth high-dimensional features, shape (B, feature_dim)
        :param features_partial: Partial input high-dimensional features, shape (B, feature_dim)
        :return: Aligned and augmented features, alignment losses
        """
        B, D = features_partial.shape

        # Split features into fragments
        features_gt_frag = features_gt.view(B, self.num_fragments, self.fragment_dim)
        features_partial_frag = features_partial.view(B, self.num_fragments, self.fragment_dim)

        aligned_fragments_partial = []
        aligned_fragments_gt = []

        lfa1, lfa2, lfa3 = 0.0, 0.0, 0.0

        for i in range(self.num_fragments):
            frag_gt = features_gt_frag[:, i, :]
            frag_partial = features_partial_frag[:, i, :]

            # Align partial fragments
            dists_partial = torch.cdist(frag_partial.unsqueeze(1), self.feature_cache.unsqueeze(0).expand(B, -1, -1))
            nearest_idx_partial = dists_partial.argmin(dim=-1).squeeze(1)
            nearest_feat_partial = self.feature_cache[nearest_idx_partial]
            aligned_fragments_partial.append(nearest_feat_partial)

            # Align gt fragments
            dists_gt = torch.cdist(frag_gt.unsqueeze(1), self.feature_cache.unsqueeze(0).expand(B, -1, -1))
            nearest_idx_gt = dists_gt.argmin(dim=-1).squeeze(1)
            nearest_feat_gt = self.feature_cache[nearest_idx_gt]
            aligned_fragments_gt.append(nearest_feat_gt)

            # Compute losses with stop-gradient
            lfa1 += F.mse_loss(nearest_feat_gt.detach(), frag_partial)
            lfa2 += F.mse_loss(nearest_feat_gt.detach(), frag_gt)
            lfa3 += F.mse_loss(frag_gt.detach(), nearest_feat_gt)

        aligned_partial = torch.stack(aligned_fragments_partial, dim=1).view(B, -1)
        aligned_gt = torch.stack(aligned_fragments_gt, dim=1).view(B, -1)



        if self.training:
            # During training, return alignment losses
            # Augment features
            augmented_features = self.augmentation_weight * features_partial + \
                                (1 - self.augmentation_weight) * aligned_gt
        else:
            augmented_features = self.augmentation_weight * features_partial + \
                    (1 - self.augmentation_weight) * aligned_partial
        # Total alignment loss
        alignment_loss = lfa1 + lfa2 + lfa3

        return augmented_features, alignment_loss
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        if self.encoder_type == 'graph':
            ...
            self.grouper_gt = DGCNN_Grouper(k = 16)
        else:
            self.grouper = SimpleEncoder(k = 32, embed_dims=512)
            self.grouper_gt = SimpleEncoder(k = 32, embed_dims=512)
        self.pos_embed = nn.Sequential(
            nn.Linear(in_chans, 128),
            nn.GELU(),
            nn.Linear(128, encoder_config.embed_dim)
        )
        self.pos_embed_gt = nn.Sequential(
            nn.Linear(in_chans, 128),
            nn.GELU(),
            nn.Linear(128, encoder_config.embed_dim)
        )  
        ...
        self.input_proj_gt = nn.Sequential(
            nn.Linear(self.grouper.num_features, 512),
            nn.GELU(),
            nn.Linear(512, encoder_config.embed_dim)
        )
        ...
        self.encoder_gt = PointTransformerEncoderEntry(encoder_config)

        self.increase_dim = nn.Sequential(
            nn.Linear(encoder_config.embed_dim, 1024),
            nn.GELU(),
            nn.Linear(1024, global_feature_dim))
        self.increase_dim_gt = nn.Sequential(
            nn.Linear(encoder_config.embed_dim, 1024),
            nn.GELU(),
            nn.Linear(1024, global_feature_dim))
        # query generator
        ...
        self.apply(self._init_weights)
        self.FAA = FeatureAlignmentAugmentation(feature_dim=1024,num_fragments=1, cache_size=1024,augmentation_weight=0.3)
    ...
    def forward(self, xyz, gt):
        ...
        if self.training:
            bs_gt = gt.size(0)
            coor_gt, f_gt = self.grouper_gt(gt, self.center_num) # b n c
            pe_gt =  self.pos_embed_gt(coor_gt)
            x_gt = self.input_proj_gt(f_gt)

            x_gt = self.encoder_gt(x_gt + pe_gt, coor_gt) # b n c
            global_feature_gt = self.increase_dim_gt(x_gt) # B 1024 N 
            global_feature_gt = torch.max(global_feature_gt, dim=1)[0] # B 1024

            out_fea, lfa = self.FAA(global_feature_gt, global_feature)
        else:
            out_fea, lfa = self.FAA(global_feature, global_feature)

        coarse = self.coarse_pred(out_fea).reshape(bs, -1, 3)
        ...
        if self.training:
            ...
            return q, coarse, denoise_length, lfa
            ...
            return q, coarse, 0, lfa

######################################## TEETHM4T ########################################  

@MODELS.register_module()
class TEETHM4T(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
    ...
    def forward(self, xyz, gt):
        q, coarse_point_cloud, denoise_length, lfa = self.base_model(xyz, gt) # B M C and B M 3
