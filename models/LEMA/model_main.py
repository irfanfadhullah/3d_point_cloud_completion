...
class Convlayer(nn.Module):
    def __init__(self, point_scales):
        ...
        self.avgpool = torch.nn.AvgPool2d((self.point_scales, 1), 1)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn2(self.conv2(x)))  # 64
        x_128 = F.relu(self.bn3(self.conv3(x)))  # 128
        x_256 = F.relu(self.bn4(self.conv4(x_128)))  # 256
        x_512 = F.relu(self.bn5(self.conv5(x_256)))  # 512
        x_1024 = F.relu(self.bn6(self.conv6(x_512)))  # 1024
        x_128_avg = torch.squeeze(self.avgpool(x_128), 2)

        x_128_max = torch.squeeze(self.maxpool(x_128), 2)
        x_256_avg = torch.squeeze(self.avgpool(x_256), 2)
        x_256_max = torch.squeeze(self.maxpool(x_256), 2)
        x_512_avg = torch.squeeze(self.avgpool(x_512), 2)
        x_512_max = torch.squeeze(self.maxpool(x_512), 2)
        x_1024_avg = torch.squeeze(self.avgpool(x_1024), 2)
        x_1024_max = torch.squeeze(self.maxpool(x_1024), 2)
        AVG = [x_512_avg, x_256_avg, x_128_avg, x_128_avg]
        MAX = [x_128_max, x_256_max, x_512_max, x_1024_max]
        AVG = torch.cat(AVG, 1)
        MAX = torch.cat(MAX, 1)
        x = torch.cat((AVG, MAX), 1)
        # x =x.permute(0,2,1)
        # print("%%%%",x.shape,x_1024.shape)
        # x =torch.bmm(x_1024,x,out=None)
        # x = x.transpose(1, 2)  # 现在 x 的尺寸是 (24, 1024, 1024)
        #
        # # 应用最大池化
        # y = F.max_pool1d(x, kernel_size=1024)  # kernel_size 是池化窗口的大小

        # 现在 y 的尺寸是 (24, 1024, 1)
        print("AVG+MAX", x.size())
        return x
...
class Latentfeature(nn.Module):
    ...
    def forward(self, x, midu):
        ...
        # print("latentfeature",latentfeature.shape)#(128+256+5121024)
        ...
        b_flattened = midu.view(midu.shape[0], -1)

        # 将 a 和展平后的 b 进行拼接
        combined_tensor = torch.cat((latentfeature, b_flattened), dim=1)
