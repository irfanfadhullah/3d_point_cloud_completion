...
def get_repulsion_loss(pred, nn_size=20, radius=0.07, h=0.03, eps=1e-12):
        #print('pred shape', pred.shape)
        idx = pn2.ball_query(radius, nn_size, pred, pred)

        #_, idx = knn_point(nn_size, pred, pred)
        #idx = idx[:, :, 1:].to(torch.int32) # remove first one
        idx = idx.contiguous() # B, N, nn

        pred = pred.transpose(1, 2).contiguous() # B, 3, N
        grouped_points = pn2.grouping_operation(pred, idx) # (B, 3, N), (B, N, nn) => (B, 3, N, nn)
